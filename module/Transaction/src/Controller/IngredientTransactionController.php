 ...
 * @property ItemTransactionService $ingrTransactionService
 * @property RetrieveByDatatableModel $datatableModel
 */
class IngredientTransactionController extends AbstractActionController
{

    /** @var $ingrTransactionService \Transaction\Service\StockTransaction\IStockTransactionService */
    private $sm;
    ...
    public function onDispatch(\Zend\Mvc\MvcEvent $e)
    {
        ...
        if ((empty($this->em->getRepository(Storage::class)->find($this->from))
                || !($this->em->getRepository(Storage::class)->getIsStuffInStorage($this->to, 'ingredient'))) ||
    ...
    protected function stockTransaction()
    {
            ...
            if (!empty($this->em->getRepository(Storage::class)->find($this->from)->getStorageType()->getIsRealStorageType())) {
                foreach ($this->em->getRepository(Stock::class)->getIngredientsAmount($this->from) as $stockRow) {
                    $datas[] = [
                       ...
                       $this->sm->get('nf')->nf($stockRow->getAmount(),true) . ' ' . $stockRow->getItem()->getIngredientUnit()->getShortName() . '<input type="hidden" class="stock-amount" id="ingr-' . $stockRow->getItem()->getId() . '" value="' . $this->sm->get('nf')->nf($stockRow->getAmount(),true) . '">',
                    ];
                }
        ...
        $this->viewModel->setTemplate('transaction/ingr-transaction/_ingr-transaction.phtml');
