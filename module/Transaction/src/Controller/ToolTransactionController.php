...
/**
 * Class ToolTransactionController
 * @package Transaction\Controller
 * @property ItemTransactionEventModel $transactionModel
 * @property ItemTransactionService $toolTransactionService
 * @property RetrieveByDatatableModel $datatableModel
 */
class ToolTransactionController extends AbstractActionController
{

    /** @var $toolTransactionService \Transaction\Service\StockTransaction\IStockTransactionService */
    private $sm;
    private $toolTransactionService;
    private $msg;
    private $actionName;
    ...
    public function __construct($sm, $misc)
    {
        ...
        $this->toolTransactionService = $misc['toolTransactionService'];
    ...
    public function onDispatch(\Zend\Mvc\MvcEvent $e)
    {
        ...
        if ((empty($this->em->getRepository(Storage::class)->find($this->from))
                || !($this->em->getRepository(Storage::class)->getIsStuffInStorage($this->to, 'tool'))) ||
            ...
            return $this->redirect()->toRoute('tooltransactionchoose', ['action' => $this->actionName]);
        }
    ...
    protected function stockTransaction()
    {
        ...
        if (!empty($postData)) {
            ...
            try {
                ...
                $this->toolTransactionService->setPostData($postData);
                $this->toolTransactionService->adjustTransactionEventModel();
                $this->toolTransactionService->setEventManager($this->getEventManager());
                $this->toolTransactionService->triggerStockTransactionEvent();
        ...
        if ($postData === false) {
            ...
            $header = ['Id', 'Eszköz'];
            $datas = [];
            //ha valódi stock, akkor csak a saját eszközt bontsuk ki
            if (!empty($this->em->getRepository(Storage::class)->find($this->from)->getStorageType()->getIsRealStorageType())) {
                foreach ($this->em->getRepository(ToolStock::class)->getToolsAmount($this->from) as $stockRow) {
                    $datas[] = [
                        $stockRow->getItem()->getId(),
                        $stockRow->getItem()->getName(),
                        $this->sm->get('nf')->nf($stockRow->getAmount(), true) . ' ' . $stockRow->getItem()->getToolUnit()->getShortName() . '<input type="hidden" class="stock-amount" id="tool-' . $stockRow->getItem()->getId() . '" value="' . $this->sm->get('nf')->nf($stockRow->getAmount(), true) . '">',
                    ];
                }
                $inputs[0]['whichColumn'] = 3;
                $header[] = 'Készleten';
            } else {
                foreach ($this->em->getRepository(Tool::class)->findAll() as $toolRow) {
                    $datas[] = [
                        $toolRow->getId(),
                        $toolRow->getName(),
                    ];
        ...
        $this->viewModel->setTemplate('transaction/tool-transaction/_tool-transaction.phtml');
    ...
    public function toolreceiveAction()
    {
        ...
        $datas = [];
        foreach ($this->em->getRepository(Tool::class)->findAll() as $aTool) {
            $datas[] = [
                $aTool->getId(),
                $aTool->getName(),
            ];
        ...
        $this->datatableModel->setData($datas);
        $this->datatableModel->setHeader(['Id', 'Eszköz', 'Mennyiség']);
    ...
    public function toolreturnAction()
    {
    ...
    public function toolmoveAction()
    {
        $storageTypeFrom = $this->em->getRepository(Storage::class)->find($this->from)->getStorageType()->getIsRealStorageType();
        $storageTypeTo = $this->em->getRepository(Storage::class)->find($this->to)->getStorageType()->getIsRealStorageType();
    ...
    public function tooldiscardAction()
    {
        $storageTypeFrom = $this->em->getRepository(Storage::class)->find($this->from)->getStorageType()->getIsRealStorageType();
        $storageTypeTo = $this->em->getRepository(Storage::class)->find($this->to)->getStorageType()->getStringId();
    ...
    public function toolstockcorrectionAction()
    {
        $storageTypeFrom = $this->em->getRepository(Storage::class)->find($this->from)->getStorageType()->getIsRealStorageType();
        $storageTypeTo = $this->em->getRepository(Storage::class)->find($this->to)->getStorageType()->getIsRealStorageType();
    ...
    public function tooluniversalAction()
    {
